# Service and Process Management 106d94888ba5472188595153e358e372
...
[Using systemctl Command \[15 Examples\]](https://linuxhandbook.com/systemctl-commands/)
...
```bash
JuiceWRLD100@htb[/htb]$ systemctl start ssh
```
...
```bash
JuiceWRLD100@htb[/htb]$ systemctl enable ssh
...
```bash
JuiceWRLD100@htb[/htb]$ ps -aux | grep ssh
...
```bash
uiceWRLD100@htb[/htb]$ systemctl list-units --type=service
...
```bash
JuiceWRLD100@htb[/htb]$ journalctl -u ssh.service --no-pager
...
```bash
JuiceWRLD100@htb[/htb]$ kill 9 <PID>
```
...
```bash
JuiceWRLD100@htb[/htb]$ ping -c 10 www.hackthebox.eu

JuiceWRLD100@htb[/htb]$ vim tmpfile
[Ctrl + Z]
...
```bash
JuiceWRLD100@htb[/htb]$ bg
JuiceWRLD100@htb[/htb]$ 
...
```bash
JuiceWRLD100@htb[/htb]$ echo '1'; echo '2'; echo '3'
1
2
3
```
...
```bash
JuiceWRLD100@htb[/htb]$ echo '1'; ls MISSING_FILE; echo '3'
1
...
```bash
systemctl list-units --type=service | grep "Load App”
```
