# Network Services 9da2e8dc74dd4df0b6bf90f03541abc5

## SSH

Secure Shell (`SSH`) is a network protocol that allows the secure transmission of data and commands over a network. It is widely used to securely manage remote systems and securely access remote systems to execute commands or transfer files.

**Install OpenSSH**

```bash
JuiceWRLD100@htb[/htb]**$** sudo apt install openssh-server -y
```

**Server Status**

```bash
JuiceWRLD100@htb[/htb]$ systemctl status ssh
...
It also offers features such as access controls, real-time file transfer, and support for multiple users accessing data simultaneously. We can use this service just like FTP in case there is no FTP client installed on the target system, or NFS is running instead of FTP.
...
```bash
JuiceWRLD100@htb[/htb]**$** sudo apt install nfs-kernel-server -y
```

**Server Status**

```bash
JuiceWRLD100@htb[/htb]$ systemctl status nfs-kernel-server
...
● nfs-server.service - NFS server and services
    ...
    Process: 9235 ExecStart=/usr/sbin/rpc.nfsd$RPCNFSDARGS (code=exited, status=0/SUCCESS)   Main PID: 9235 (code=exited, status=0/SUCCESS)
        CPU: 10ms
```
...
Some of the most popular web servers for Linux servers are Apache, Nginx, Lighttpd, and Caddy.

We can use a web server to perform file transfers, allowing us to log in and interact with a target system through an incoming HTTP or HTTPS port. Finally, we can use a web server to perform phishing attacks by hosting a copy of the target page on our own server and then attempting to steal user credentials.

**Install Apache Web Server**

```bash
JuiceWRLD100@htb[/htb]**$** sudo apt install apache2 -y
```
...
```bash
JuiceWRLD100@htb[/htb]$ sudo apt install python3 -y
JuiceWRLD100@htb[/htb]$ python3 -m http.server
```
...
```bash
JuiceWRLD100@htb[/htb]**$** python3 -m http.server --directory /home/cry0l1t3/target_files
```
...
```bash
JuiceWRLD100@htb[/htb]**$** python3 -m http.server 443
```
...
```bash
JuiceWRLD100@htb[/htb]**$** sudo apt install openvpn -y
```
...
```bash
JuiceWRLD100@htb[/htb]**$** sudo openvpn --config internal.ovpn
```
